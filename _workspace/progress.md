...
| - | 소셜로그인 | FR-AUTH-001 카카오·Google 실연동: 카카오 개발자 콘솔·Google Cloud Console 앱 등록(브라우저 조작)부터 BE(`SocialOAuthClient`+Kakao/Google 구현체, `User.provider/providerId`, `POST /auth/kakao·google`)·FE(`api/oauth.ts` 인가 URL, `/oauth/callback/:provider` 콜백 페이지, 로그인 버튼 실연결)까지 전 구간 구현. 시크릿은 `application-secrets.yml`(gitignore)+prod 환경변수. 네이버 제외(범위 밖) | DONE — 실제 카카오·Google 계정으로 브라우저 E2E 로그인 성공(자동가입→JWT 발급→홈 진입), 카카오 더미 이메일/닉네임·Google 실이메일/실명 모두 마이페이지에서 확인 |
| - | 계정연동 | 소셜 로그인 이메일 충돌 시 에러(VALID_001) 대신 기존 계정에 연결하도록 개선: `SocialAccount`(userId+provider+providerId) 신규 엔티티로 `User.providerId` 단일컬럼을 대체(1유저 다중 제공자 지원), `AuthService.linkOrCreateUser`, Google `email_verified` 검증 후에만 연동 근거로 사용(미검증 이메일 신뢰 금지). `MyPageService.deleteAccount`의 소셜전용계정(비밀번호 null) NPE성 500도 함께 방어 | DONE — 로컬 회원가입(bchi2000@gmail.com) 후 같은 Google 계정으로 로그인 시 신규가입 아닌 기존 계정(userId=1, nickname 유지)에 연동됨을 실검증, 연동 후 로컬 비밀번호 로그인도 정상 동작 확인. 부수 발견: React 18 StrictMode dev 이중 mount가 1회용 인가코드를 두 번 소진하는 레이스 버그 — `OAuthCallbackPage`의 useRef 가드를 모듈스코프 Set으로 교체해 수정 |
| - | 카카오비즈앱 | 카카오 개인 개발자 비즈 앱 전환(사용자가 본인인증+약관동의 직접 수행) 후, 앱 아이콘 등록(브랜드 컬러 SVG→128px PNG 래스터화)·동의항목에서 카카오계정(이메일)을 필수 동의로 설정(+ 계정에 이메일 없을 시 입력 요청 수집 옵션 활성화) | DONE — 코드 변경 없이(기존 `info.email()` 우선 로직 그대로) DB 초기화 후 재로그인 시 더미 이메일이 아닌 실제 이메일(bchi2000@gmail.com)이 저장됨을 실검증 |
